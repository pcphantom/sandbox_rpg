...
from constants import (
    ...
    CHEST_CAPACITY, WAVE_SPAWN_RADIUS, PLACEMENT_RANGE,
)
...
# -- Texture keys for placement ghost preview --------------------------------
PLACEABLE_TEXTURE_KEYS: Dict[str, str] = {
    'campfire': 'campfire_True',
    'torch': 'torch_placed',
    'trap': 'trap_placed',
    'bed': 'bed_placed',
    'wall': 'wall_placed',
    'stone_wall_b': 'stone_wall_placed',
    'turret': 'turret_placed',
    'chest': 'chest_placed',
    'door': 'door_placed',
}
...
class Game:
    def __init__(self) -> None:
        ...
        self.placing_item: Optional[str] = None
        ...
        for event in pygame.event.get():
            ...
            if self.placing_item:
                if event.type == pygame.KEYDOWN and event.key == pygame.K_ESCAPE:
                    self.placing_item = None
                    self._notify("Placement cancelled.")
    ...
    def _use_equipped_item(self) -> None:
        ...
        if heal > 0:
            ...
            self.placing_item = eq_id
            self.show_inventory = False
            self.show_crafting = False
            self.show_character = False
            self.show_chest = False
            self.active_chest = None
            self._notify("Left-click to place, Escape to cancel.")

    def _confirm_placement(self) -> None:
        """Validate cursor position and place the item if valid."""
        if not self.placing_item:
            return
        item_id: str = self.placing_item
        mx, my = pygame.mouse.get_pos()
        world_x = mx + self.camera.x
        world_y = my + self.camera.y

        pt: Transform = self.em.get_component(self.player_id, Transform)
        dist = math.hypot(world_x - pt.x, world_y - pt.y)
        if dist > PLACEMENT_RANGE:
            self._notify("Too far away!")
            return
        ...
        self._place_item(item_id, world_x, world_y)
        inv.remove_item(item_id, 1)
        self._notify(f"Placed {ITEM_DATA[item_id][0]}")

        if not inv.has(item_id):
            self.placing_item = None

    def _place_item(self, item_id: str, px: float, py: float) -> None:
        eid = self.em.create_entity()
    ...
    def _build_save_data(self) -> Dict[str, Any]:
        ...
        # Save world resources (trees/rocks)
        resources: list[dict[str, Any]] = []
        tree_tex = self.textures.get('tree')
        rock_tex = self.textures.get('rock')
        for eid in self.em.get_entities_with(Transform, Renderable, Collider):
            ...
            if self.em.has_component(eid, Placeable):
                continue
            r = self.em.get_component(eid, Renderable)
            t = self.em.get_component(eid, Transform)
            if r.surface is tree_tex:
                resources.append({'type': 'tree', 'x': t.x, 'y': t.y})
            elif r.surface is rock_tex:
                resources.append({'type': 'rock', 'x': t.x, 'y': t.y})
    ...
    def _apply_save_data(self, data: Dict[str, Any]) -> None:
        ...
        # Restore world tiles if saved
        if 'tiles' in data:
            self.world.tiles = data['tiles']

        # Restore resources (trees/rocks)
        if 'resources' in data:
            tree_tex = self.textures.get('tree')
            rock_tex = self.textures.get('rock')
            # Remove existing trees/rocks
            for eid in list(self.em.get_entities_with(
                ...
                if self.em.has_component(eid, Placeable):
                    continue
                r = self.em.get_component(eid, Renderable)
                if r.surface is tree_tex or r.surface is rock_tex:
                    self.em.destroy_entity(eid)
            # Recreate from save data
            for res in data['resources']:
                eid = self.em.create_entity()
                self.em.add_component(eid, Transform(res['x'], res['y']))
                if res['type'] == 'tree':
                    self.em.add_component(eid, Renderable(tree_tex, layer=2))
                    self.em.add_component(eid, Collider(24, 32, True))
                elif res['type'] == 'rock':
                    self.em.add_component(eid, Renderable(rock_tex, layer=1))
                    self.em.add_component(eid, Collider(26, 18, True))
    ...
    def _draw_placement_preview(self) -> None:
        if not self.placing_item:
            return
        tex_key = PLACEABLE_TEXTURE_KEYS.get(self.placing_item)
        if not tex_key:
            return
        ...
        tile_x = int(world_x // TILE_SIZE)
        tile_y = int(world_y // TILE_SIZE)
        solid = self.world.is_solid(tile_x, tile_y)
        valid = dist <= PLACEMENT_RANGE and not solid

        ghost = tex.copy()
        tint = pygame.Surface(ghost.get_size(), pygame.SRCALPHA)
        tint.fill((0, 255, 0, 80) if valid else (255, 0, 0, 80))
        ghost.blit(tint, (0, 0))
        ghost.set_alpha(160)

        draw_x = mx - ghost.get_width() // 2
        draw_y = my - ghost.get_height() // 2
        self.screen.blit(ghost, (draw_x, draw_y))

        # Range circle around player (screen coords)
        pcx = int(pt.x - self.camera.x + 10)
        pcy = int(pt.y - self.camera.y + 14)
        range_surf = pygame.Surface((SCREEN_WIDTH, SCREEN_HEIGHT), pygame.SRCALPHA)
        pygame.draw.circle(range_surf, (100, 255, 100, 30),
                           (pcx, pcy), int(PLACEMENT_RANGE), 2)
