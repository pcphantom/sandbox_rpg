...
ITEM_DATA: Dict[str, Tuple[str, str, int, int, int, bool]] = {
    ...
    'sword':    ('Wood Sword',   'Best melee damage.',                20, 0,  0, False),
...
class Game:
    ...
    def _create_player(self) -> int:
        ...
        self.em.add_component(eid, Inventory(24))
    ...
    def _spawn_mob(self) -> None:
        ...
        for _ in range(20):
            ...
            if self.world.is_solid(x, y):
                continue
            if self.daynight.get_darkness() > 0.4 and random.random() < 0.5:
    ...
    def _update(self, dt: float) -> None:
        ...
        for eid in list(self.em.get_entities_with(Health, AI)):
            ...
            if not h.is_alive():
                ...
                color = (50, 200, 70) if mob_ai.mob_type == 'slime' else (200, 200, 210)
    ...
    def _get_attack_damage(self) -> int:
        ...
        eq = inv.get_equipped()
        base = 5
        if eq and eq in ITEM_DATA and ITEM_DATA[eq][2] > 0:
            base = ITEM_DATA[eq][2]
        return base + (ps.level - 1) * 2
    ...
    def _interact(self) -> None:
        ...
        if nearest is not None:
            ...
            eq = inv.get_equipped()
            bonus = ITEM_DATA[eq][3] if eq and eq in ITEM_DATA else 0
    ...
    def _draw_hotbar(self) -> None:
        ...
        if eq and eq in ITEM_DATA:
            nt = self.font.render(ITEM_DATA[eq][0], True, (220, 220, 240))
    ...
    def _draw_crafting(self) -> None:
        ...
        for i, recipe in enumerate(RECIPES):
            ...
            for item, cost in recipe['cost'].items():
                ...
                ct = self.font_sm.render(f"{cost}x{ITEM_DATA[item][0]}", True, c_color)
